...
class Sportsmen(ABC):
    competition_level: list[str] = [
        "Областной", "Международный", "Чемпионат мира"
    ]
    ...
    @classmethod
    def create_sportsmen(cls, data: dict) -> Union[
        "TeamSportsmen", "IndividualSportsmen"
    ]:
        if data["type_sport"] in ["Футбол", "Баскетбол", "Волейбол"]:
            return TeamSportsmen(
                data["name"], data["age"], data["type_sport"], data["team"]
            )
        return IndividualSportsmen(
            data["name"], data["age"], data["type_sport"], data["record"]
        )
...
sportsmen_data1: dict = {
    "name": "Евгений Рылов", "age": 28,
    "type_sport": "Плавание", "record": 23.7
}
sportsmen_data2: dict = {
    "name": "Денис Еремеев", "age": 19, "type_sport": "Футбол", "team": "Волга"
}
new_sportsmen1 = Sportsmen.create_sportsmen(sportsmen_data1)
new_sportsmen2 = Sportsmen.create_sportsmen(sportsmen_data2)
print(
    (new_sportsmen1.name, new_sportsmen1.age),
